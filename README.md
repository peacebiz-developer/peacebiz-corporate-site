...
- The workflow builds with `CI=false` to prevent existing ESLint warnings from being treated as hard errors in GitHub Actions.
...
### GitHub Actions build fails with "Treating warnings as errors because process.env.CI = true"

- Create React App treats warnings as errors in CI by default.
- This repository workflow sets `CI=false` for the build step so deployment can proceed while warnings are cleaned up incrementally.
